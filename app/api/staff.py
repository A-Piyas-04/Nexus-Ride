...
def update_staff_profile(
    ...
    if data.mobile_number is not None:
        if data.mobile_number:
            existing_user = session.exec(
                select(User).where(User.mobile_number == data.mobile_number).where(User.id != current_user.id)
            ).first()
            if existing_user:
    ...
    if data.default_route_name is not None:
        if data.default_route_name == "" or data.default_route_name is None:
    ...
    if data.default_pickup_stop_name is not None:
        if data.default_pickup_stop_name == "" or data.default_pickup_stop_name is None:
